  ...
  it('ログインのためのリンクが含まれる', (done) => {
    return (
      request(app)
        ...
        // expect 関数に、正規表現を 1 つ渡すと、 HTML の body 内にその正規表現が含まれるかをテスト
        .expect(/<a class="btn btn-info my-3" href="\/auth\/github"/)
        // テストを終了する際には expect 関数に、期待されるステータスコードの整数と
        // テスト自体の引数に渡される done 関数を渡します
        .expect(200, done)
    );
  });
        ...
        .end((err, res) => {
          ...
          promiseDeleted.then(() => {
            ...
            const p2 = Availability.findAll({
              ...
              // TODO テストを実装
              assert.deepEqual(availabilities.length, 0);
            });
            const p3 = Candidate.findAll({
              where: { scheduleId: scheduleId },
            }).then((candidates) => {
              // TODO テストを実装
              assert.deepEqual(candidates.length, 0);
            });
            const p4 = Schedule.findByPk(scheduleId).then((schedule) => {
              // TODO テストを実装
              assert.deepEqual(!schedule, true);
            });
